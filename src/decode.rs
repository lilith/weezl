    ...
    for DecodeStateStreaming<P, CgC>
{
    ...
    fn advance(&mut self, mut inp: &[u8], mut out: &mut [u8]) -> BufferResult {
        ...
        // Reference into `out` pointing at the previous code's written
        // value. Used by the KwKwK path to memcpy the previous output
        // instead of walking the table chain. None when the previous
        // code went to pending, came from a prior advance() call, or
        // was written inside the mini-burst fast path (which doesn't
        // maintain this reference to keep its inner loop tight).
        //
        // Matches the `last_decoded` pattern used by the Classic burst
        // decoder — the reference lives alongside the shrinking `out`
        // because `split_at_mut` gives us disjoint mutable slices.
        let mut last_decoded: Option<&[u8]> = None;
        ...
        loop {
            ...
            if code < self.clear_code {
                ...
                out[0] = code as u8;
                out = &mut out[1..];
                // A literal writes 1 byte, which is ≤ Q. The KwKwK path
                // handles short prev values by reading the table suffix
                // directly — no `last_decoded` reference needed here.
                ...
                // The mini-burst writes only short values (literal = 1 byte,
                // short copy ≤ Q bytes). Since a KwKwK's `prev_len` after
                // the mini-burst is guaranteed ≤ Q, the KwKwK path's
                // short-prev table read handles it — no `last_decoded`
                // tracking required inside this loop.
