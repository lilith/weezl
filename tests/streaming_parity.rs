...
//! and Streaming (and Chunked where relevant). The outputs must be byte-for-
...
fn encode(data: &[u8], order: BitOrder, size: u8, tiff: bool) -> Option<Vec<u8>> {
    ...
    let mut buf = Vec::with_capacity(data.len() * 2 + 64);
    match encoder.into_stream(&mut buf).encode_all(data).status {
        Ok(_) => Some(buf),
        Err(_) => None,
    }
}
